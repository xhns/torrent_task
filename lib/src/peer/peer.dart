...
class _UTPPeer extends Peer {
  ...
  @override
  Future dispose([reason]) async {
    // Именно closeForce, а не close: `close()` шлёт FIN и ЖДЁТ подтверждения
    // от удалённой стороны. Для ПРИНЯТОГО (входящего) uTP-сокета это
    // катастрофа при остановке задачи — качающий, оборвавший соединение
    // первым, никогда не подтвердит наш FIN, а `PeersManager.dispose`
    // разбирает пиров последовательно: одна такая пауза множится на число
    // пиров. На живом стенде `task.stop()` после входящего uTP не завершался
    // вовсе.
    //
    // Вежливость по отношению к пиру, которого почти наверняка уже нет, не
    // стоит зависшей остановки приложения.
    // Покрыто: test/incoming_utp_test.dart (задача останавливается за
    // разумное время после входящего uTP).
    await _socket?.closeForce();
    await _client?.close();
    return super.dispose(reason);
  }
}
